...
在视频接入中选择摄像头，打开具体服务的服务配置，进入“告警图片隐私保护”。默认关闭；启用后可选择打码类别和马赛克强度。设置按摄像头及服务保存，不需要另外部署检测算法。

系统使用告警图片同一帧的完整检测框，对选中类别的整个目标打马赛克，包括告警区域外和未触发告警的目标。红色告警框仍表达原来的告警含义。打码在图片编码、保存和推送之前完成，告警场景图及其目标裁剪图均使用处理后的副本。

如果缺少同帧完整检测结果、所选类别不可用或图片处理失败，保留告警事件，但不保存或提供受影响的图片。有效的零目标检测与检测结果缺失会分别处理。模型漏检的目标无法自动打码，请用实际场景确认覆盖效果。

此设置仅作用于新告警图片，不处理历史告警、实时预览、录像、发给外部大模型的请求图片或注册底库图片。开启前请按实际业务确认这些输出的使用范围。
