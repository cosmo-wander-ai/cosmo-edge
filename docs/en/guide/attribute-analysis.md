...
3. Set the accumulator purpose to **Object attributes**. Give each attribute a unique key, display name, upstream classifier source, single/multiple-value type, confidence threshold and required vote share.
4. Map raw model labels to value keys and display names, for example `wearing_hat → yes → Wearing a hat` and `no_hat → no → No hat`. A negative value needs an explicit model output; missing evidence is not a negative.
...
Missing or insufficient evidence is **Unknown**. With no valid samples and an observed inference failure, the result is **Inference failed**. Neither becomes an implicit negative. Confidence is the mean confidence of winning-label observations, averaged across winning values for multiple-value attributes.
