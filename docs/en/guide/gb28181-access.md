...
CosmoEdge owns SIP/TCP and SIP/UDP registration, Digest MD5 authentication, heartbeats, Catalog and INVITE/ACK/BYE. TCP media enters SRS directly; managed UDP ingress validates/reorders RTP and feeds the same SRS PS pipeline through loopback TCP. SRS converts it to `rtmp://127.0.0.1:1936/live/<video-channel-id>`. Existing demux, decode, algorithm, preview, snapshot, recording and alarm paths are reused. Automated UDP coverage is not cross-vendor device acceptance.
...
- SIP listens on both TCP and UDP and responds using the registration transport. Media is configured independently, defaulting to camera-initiated TCP; UDP unicast is optional. This is a bounded direct-device SIP subset, not a general SIP proxy.
- At most 128 registrations, 64 concurrent TCP SIP connections, 128 UDP SIP peers, 1,024 entries per device catalog and 256 media demands. Existing channel/decoder limits still apply.
- H.264 is preferred for validation. H.265 needs camera-packaging, target-hardware and browser verification.
- No UDP multicast media, platform-initiated media TCP, SIP TLS, SHA-256 Digest, Record-Route/proxy/cascading, catalog subscriptions, PTZ, audio business features or GB recording search/playback. Missing catalogs do not trigger guessed channels.
...
| SIP server port | Platform → SIP server port | 5060 by default, both TCP and UDP |
...
| Transport | Automatic for SIP; Advanced device settings → Video transport | Video defaults to TCP, select UDP for UDP media; independent of SIP transport |
...
The explicit per-device **skip password verification** option is disabled by default and intended only for trusted isolated networks. An ID match does not authenticate a physical device. Unlisted IDs remain blocked. HTTP management, SIP/TCP and SIP/UDP do not provide transport encryption; do not expose them directly to the Internet.
...
3. Configure matching fields on the camera and enable GB28181 using either TCP or UDP SIP. Video defaults to TCP; select UDP in Advanced device settings for UDP media. Transport changes require re-registration.
...
Legacy `COSMO_GB28181_ENABLED`, `COSMO_GB28181_SIP_PORT` and `COSMO_GB28181_CANDIDATE` no longer control managed SIP; saved UI settings are authoritative. `COSMO_GB28181_MEDIA_PORT` still controls SRS media reception, TCP 9001 by default; managed UDP ingress binds the same numbered UDP port on demand. Avoid WebSocket port 9000. Both SIP and media ports must be reachable. Existing devices without a media transport field retain TCP.

Bridge deployments can add the optional `docker-compose.gb28181.yml` overlay to publish both transports. `COSMO_GB_SIP_PORT` defaults to 5060 and must match the platform UI; `COSMO_GB_MEDIA_PORT` defaults to 9001. Choose unused ports when sharing a host with another GB platform, update cameras accordingly, and explicitly advertise the camera-reachable host IPv4. This does not fix ONVIF multicast discovery or change host firewall rules.

UDP media accepts only allocated SSRCs from the registration source IP and pins the first valid media source port. Separate signaling/media source IPs are outside this version's scope. Reordering is bounded to 64 packets and approximately 50 ms; per-stream reorder and pending-write buffers are capped at 128 KiB and 256 KiB. A full reorder buffer drops excess packets; a full pending-write buffer fails the stream for existing retry/reallocation logic. Missing RTP sequence numbers are preserved, not filled with fabricated data; the PS parser discards incomplete data and resynchronizes. Lossy networks may need a later decodable keyframe; UDP does not guarantee lossless playback or a fixed first-frame delay.
...
| Waiting for media / timeout | SIP server address, selected TCP/UDP media port mappings and firewall, device video transport; TCP requires camera-active connection |
