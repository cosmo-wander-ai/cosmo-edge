...
void AlgChannelDemux::AddViewerPacketQueue(std::shared_ptr<AsyncQueue<VideoPacketPtr>> async_packet_queue) {
    ...
    if (it == async_packet_queues_.end()) {
        ...
        // A cached I frame alone cannot decode current P frames: their
        // references include intervening packets. Passthrough starts at the
        // next live IDR; a timed-out viewer can use the warm decoder instead.
    }
}
