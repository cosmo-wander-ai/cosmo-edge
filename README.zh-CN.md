...
认证设备包可以包含经过 VLM-as-judge 场景验证的 `CosmoEdge-VL-Judge-0.8B` 端侧视觉判断模型和生产可用的 CV 模型。开源引擎也可以通过同一套工作流运行用户自带模型。
...
| 视觉状态判断 | `CosmoEdge-VL-Judge-0.8B`（文本提示词 -> YES/NO/枚举） | 异步流水线节点 |
| 图片分析 | VLM 批量分析 | 独立任务 |

**端侧 VLM 模型包：**

`CosmoEdge-VL-Judge-0.8B` 是认证设备包中的验证模型包，基于 Qwen3.5 0.8B 进行视觉状态判断场景微调和端侧量化，面向封闭式 VLM-as-judge 任务，例如 YES/NO 和枚举状态输出。开源仓库提供引擎、流水线节点和模型接入路径；社区量化权重或用户自带 VLM 需要在目标场景中自行验证，不作为默认生产可用模型承诺。
...
开源仓库重点提供引擎、UI、流水线和模型接入路径，不默认包含生产模型权重。你可以自带模型，并参考模型适配指南。认证设备包可以包含预安装的生产 CV 模型、经过验证的 `CosmoEdge-VL-Judge-0.8B` 和 GroundingDINO；社区量化权重或自带 VLM 需要按目标场景自行验证。
